<template>
    <div class="holders incidents-holder ">
        <queryNavbar @input-changed="onInputChanged" :theme="'flashy'" >
            ...
            <template #options>
                <p v-if="AuthStore.userType == 'Admin' " @click="$router.push({name : 'scriptCreate'})" >create+</p>
            </template>
        </queryNavbar>
        <tableSimpleHeader v-if="fetchingScripts == false" >
            ...
            <template  #tableRows >
                <tr  v-for="script in scripts" :key="script.id" >
                    ...
                    <div class="user-actions" >
                        <div @click="$router.push({name : 'scriptShow' , params : { id : script.id}})" class="user-action" >
                        ...
                        <div v-if="AuthStore.userType == 'Admin'" @click="deleteScript(script)" class="user-action" >
        ...
        <tableSkeleton v-if="fetchingScripts == true" ></tableSkeleton>
    </div>
</template>
<script>
...
export default{
    ...
    methods : {
        onInputChanged(input) {
            this.getScripts(input);
        },
        getScripts(query_name) {
            this.fetchingScripts = true;
            ...
            apiFetch.get(endpoint, false)
                .then((scripts) => {
                    ...
                    if(query_name != null && scripts.length == 0){
                        this.emitter.emit("sidebar-error",{error : true,description : "No script matching "+query_name+" was found."})
                    }
                    this.fetchingScripts = false;
                })
        ...
        deleteScript(script){
            let endpoint = '/script/delete';
            apiFetch.delete(endpoint,{id : script.id}, true, false)
            .then(()=>{
                this.$router.go();
            })
        }
