<template>
    ...
    <interrationLayout :forbiddenInput="false" >
        <template #Header>
            {{ script.name + ' script'}} 
        </template>
        <template #InputName>
            <input  type="text" placeholder="Script name..." v-model="script.name">
        </template>
        ...
        <template #InputDescription>
            <textarea  maxlength="150" v-model="script.description"></textarea>
        </template>
        <template #InputAdd>
            <label @click="addInput()" style="cursor: pointer;">Add+</label>
        </template>
        <template #Inputs>
            <div v-if="script.inputs && script.inputs.length != 0" class="global-inputs">
        ...
        <template #CodeEditor>
            <CodeEditor v-model="script.content" :read-only="false" :line-nums="true" :header="true"
                :display-language="true" :copy-code="true" width="100%" max-width="100%" height="100%"
                :autofocus="false" theme="hybrid"></CodeEditor>
        </template>
        <template #Footer>
            <buttonValidation @click="update()" :ok="true" :type="'submit'">
                <template #textButton>
                    Save changes
                </template>
            </buttonValidation>
        ...
        <template #popUps>
            <popupDisplay v-if="popUpBoolean" @close-popup="popUpBoolean = false;">
                <template #titel>New input</template>
                <template #content>
                    ...
                    <buttonValidation :ok="true" @click="pushInput" style=" padding: 2px;width: 50%;align-self: flex-end;" >
                        <template #textButton>
                            Add
                        </template>
                    </buttonValidation> 
...
export default{
    ...
    methods : {
        ...
        update(){
            this.keepUpdatedValue(this.oldScript,this.script,['id',"inputs"]);
            let endpoint = "/script/edit";
            apiFetch.update(endpoint,
            {...this.script},
            true,
            false)
            .then(()=>{
                this.$router.push({name : 'scriptShow' , params : { id : this.script.id }});
            })
        }
    },
    created(){
