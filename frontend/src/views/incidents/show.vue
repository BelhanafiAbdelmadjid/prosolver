<template>
    <interractionLayout :forbiddenSteps="false" :forbiddenInputs="true" >
        <template #IncidentViewTitel>
            {{ incident.name }}
        </template>
        <template #InputName>
            <input disabled type="text" placeholder="Incident name..." v-model="incident.name">
        </template>
        <template #InputDescription>
            <textarea disabled v-model="incident.description" maxlength="150"></textarea>
        </template>
        ...
        <template #Steps>
            <cardRoute style="width: 100%;height: fit-content;" v-for="step in incident.steps" :key="step.id" >
                <template #Titre >
                    {{ step.name }}
                </template>
                <template #icon >
                    <RouterLink    :to="{ name: 'stepShow' , params : { id : incident.id , stepID : step.id }  }"  >
                ...
                <template #Description >
                    {{ 'Executing '+step.script.name+' on ' + (step.onServer == true ? step.server.name : step.container.name + ' container of ' + step.server.name  )   }}
                </template>
            </cardRoute>

        </template>
        <template #Footer>
            <buttonValidation :warning="true" @click="$router.push({name : 'incidentEdit' , params : {id : incident.id}, query : {name : incident.name , description : incident.description}});this.emitter.emit('sidebar-error', {warning : true ,error : null, description :'You have entered edit mode.'})" :type="'Submit'">
                <template #textButton>
                    Edit
                </template>
            </buttonValidation>
